...
프로젝트 생성 및 기본 세팅
Expo 프로젝트 생성
bashnpx create-expo-app APP_MVP
...
.env 환경변수 파일 추가 후 EXPO*PUBLIC* 접두사 적용
...
javascriptallow read, write: if request.auth != null;
...
최종 수정된 업로드 로직
javascriptconst response = await fetch(manipulated.uri);
...
2025-10-09 (14:32)
게시글 작성 후 무한 로딩 문제 해결 시도
문제 현상

게시글 작성 완료 후 Firestore에 정상 등록됨
홈 화면에서 게시글 목록에도 정상 표시됨
하지만 게시글 작성 화면이 무한 로딩 상태로 유지됨
router.replace("/(tabs)")가 실행되지 않음

원인 분석

Alert.alert() 호출 후 router.replace() 실행으로 인한 비동기 타이밍 문제
상태 초기화가 화면 이동 후에 실행되어 로딩 상태가 해제되지 않음
Alert 팝업이 화면 전환을 방해하는 경우 발생

시도한 해결 방법
첫 번째 시도 (14:35)

handleSubmit 함수 내부 로직 수정
상태 초기화를 router.replace() 호출 전으로 이동

javascript// 상태 초기화 (화면 이동 전)
setTitle("");
setContent("");
setImageUri(null);
setSubmitting(false);
...
Alert.alert() 제거하여 즉시 화면 전환 실행
성공 알림 없이 바로 홈으로 이동하도록 변경

코드 변경 사항

기존: Alert.alert() 표시 후 router.replace() 호출
변경: Alert.alert() 제거, 상태 초기화 후 즉시 router.replace() 호출

예상 결과

게시글 등록 후 즉시 홈 화면으로 전환
로딩 상태 정상 해제
다음 게시글 작성 시 입력 필드 초기화 상태 확인

추가 작업 (14:40)

전체 코드에 상세 주석 추가 (한글)
모든 함수와 변수에 대한 설명 주석 작성
이모지 제거
NewPostScreen.tsx 전체 수정본 완성

현재 상태 (2025-10-09 14:45)

코드 수정 완료
실제 테스트 대기 중
무한 로딩 문제 해결 여부 확인 필요
...
현재 상태 요약 (2025-10-09 기준)
항목상태Firebase Auth (로그인/회원가입)완료Firestore CRUD (게시글/댓글)완료Firebase Storage (이미지 업로드)정상 작동iOS SafeArea 대응완료Expo 환경 변수 (.env)기존 Storage 주소 그대로 사용Expo Go 테스트이미지 업로드 정상 작동 확인게시글 작성 후 무한 로딩미해결 - 디버깅 진행 중
...
1.  무한 로딩 문제 추가 디버깅
    ...
    // ... 중간 코드 ...
        try {
            console.log("2. Firestore 등록 시작");
            await addDoc(collection(db, "posts"), { ... });
            console.log("3. Firestore 등록 완료");

            console.log("4. 상태 초기화 시작");
            setTitle("");
            setContent("");
            setImageUri(null);
            setSubmitting(false);
            console.log("5. 상태 초기화 완료");

            console.log("6. 라우터 이동 시작");
            router.replace("/(tabs)");
            console.log("7. 라우터 이동 호출 완료");
        } catch (e) {
            console.error("오류 발생:", e);
        }
    };
2.  대안 방법 시도
    javascript// 방법 1: router.back() 사용
    router.back();
...
setTimeout(() => {
router.replace("/(tabs)");
}, 100); 3. Expo Router 버전 확인

Expo Router의 알려진 이슈 확인
필요시 버전 업데이트 검토
navigation.reset() 사용 검토
